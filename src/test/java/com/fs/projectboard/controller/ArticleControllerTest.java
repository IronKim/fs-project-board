...
import com.fs.projectboard.dto.HashtagDto;
...
import com.fs.projectboard.util.FormDataEncoder;
...
@Import({TestSecurityConfig.class, FormDataEncoder.class}) // @Import는 테스트에 필요한 설정 클래스를 임포트한다.
...
class ArticleControllerTest {
    ...
    private final FormDataEncoder formDataEncoder; // FormDataEncoder는 테스트에 필요한 FormData를 인코딩하는 클래스이다.
    ...
    void givenNothing_whenRequestingArticlesView_thenReturnsArticlesView() throws Exception {
        ...
        mvc.perform(get("/articles")) // GET /articles 요청을 보낸다.
                ...
                .andExpect(model().attributeExists("searchTypes")); // articles 속성이 존재하는지 검증한다.
    ...
    @WithMockUser // WithMockUser는 테스트에 필요한 사용자 정보를 설정한다.
    ...
    void givenNothing_whenRequestingArticleView_thenReturnsArticleView() throws Exception {
        ...
        mvc.perform(get("/articles/" + articleId)) // GET /articles/{articleId} 요청을 보낸다.
                ...
                .andExpect(model().attributeExists("articleComments")) // articleComments 속성이 존재하는지 검증한다.
                .andExpect(model().attribute("totalCount", totalCount)) // totalCount 속성이 존재하는지 검증한다.
                .andExpect(model().attribute("searchTypeHashtag", SearchType.HASHTAG)); // searchTypeHashtag 속성이 존재하는지 검증한다.
    ...
    @WithUserDetails(value = "unoTest", setupBefore = TestExecutionEvent.TEST_EXECUTION) // WithUserDetails는 테스트에 필요한 사용자 정보를 설정한다.
